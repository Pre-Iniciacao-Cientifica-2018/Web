<?php
    ...
    function getMedias(){
        ...
        if (isset($_SESSION["datepicker"]) && !empty($_SESSION["datepicker"])) {
            convertToArray($return,"SELECT avg(concentracao),hour(data_hora) FROM DADOS WHERE date_format(date(data_hora),'%d/%m/%Y') like '". $_SESSION['datepicker']."' GROUP BY hour(data_hora) ORDER BY HOUR(data_hora)");        
        }
